  ...
  useEffect(() => {
    const checkUsernameUniqueness = async () => {
      if (username) {
        ...
        setUsernameMessage("");
        try {
          const response = await axios.get(`/api/check-username-uniue?username=${username}`);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <p className={`text-sm ${usernameMessage.includes("valid") ? 'text-green-500' : 'text-red-600'}`}>
                    {usernameMessage}
                  </p>
                  <FormMessage />
                </FormItem>
